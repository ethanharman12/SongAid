...
playTone = (frequency, type, duration) => {
	...
	if (typeof frequency === "string") {
		...
		//TODO: if typeof frequency[0] = number ...
		//o.frequency.value = frequency[0];
		//completeChord(frequency[1], type, duration);
		//completeChord(frequency[2], type, duration);
		o.frequency.value = tone[frequency[0].replace('s', '#') + '4'];
		completeChord(tone[frequency[1].replace('s', '#') + '4'], type, duration);
		completeChord(tone[frequency[2].replace('s', '#') + '4'], type, duration);
	} else {
		o.frequency.value = frequency;
	}
	g.connect(context.destination);
	o.start(0);
	var volume = VOLUME_CURVE;
	//if (frequency < 300) {
	//	volume = [1, .95];
 //   }
    g.gain.setValueCurveAtTime(volume, context.currentTime, duration);
}
